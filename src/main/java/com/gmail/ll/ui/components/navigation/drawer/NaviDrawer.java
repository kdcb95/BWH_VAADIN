...
public class NaviDrawer extends Div
	...
	private void initHeader() {
		mainContent.add(new BrandExpression("Menú Principal"));
	}
	...
	private void initFooter() {
		railButton = UIUtils.createSmallButton("Desanclar", VaadinIcon.CHEVRON_LEFT_SMALL);
		railButton.addClassName(CLASS_NAME + "__footer");
		railButton.addClickListener(event -> toggleRailMode());
		railButton.getElement().setAttribute("aria-label", "Minimizar menu");
	...
	private void toggleRailMode() {
		if (getElement().hasAttribute(RAIL)) {
			...
			railButton.setText("Desanclar");
			UIUtils.setAriaLabel("Minimizar Menu", railButton);

		} else {
			getElement().setAttribute(RAIL, true);
			railButton.setIcon(new Icon(VaadinIcon.CHEVRON_RIGHT_SMALL));
			railButton.setText("Anclar");
			UIUtils.setAriaLabel("Expandir", railButton);
